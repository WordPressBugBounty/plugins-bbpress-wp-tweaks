...
if (!class_exists('BbpressWpTweaks')) :
    ...
    class BbpressWpTweaks
    {
        ...
        private static $is_block_theme;
        private static $css_users_widget = '<style type="text/css" id="bbpress-wp-tweaks-users">/* users widget with count */.bbwptw-users-count{font-size: 90%; opacity: .8; margin-left: 10px;}</style>';
        private static $css_forum_description = '<style type="text/css" id="bbpress-wp-tweaks-desription">/* forum description or topic added */.bbwptw-description,.bbwptw-topic{	clear: both;float: none;margin: 1rem 0;}</style>';
        ...
        /**
         * Add (Users widget) inline CSS once to front end. Added when users widget used.
         *
         * @return string
         */
        public static function css_once_users_widget()
        {
            $return = self::$css_users_widget;
            self::$css_users_widget = '';
            return $return;
        }

        /**
         * Add (description) inline CSS once to front end. Added when forum description shown on topic and reply pages.
         *
         * @return string
         */
        public static function css_once_forum_description()
        {
            $return = self::$css_forum_description;
            self::$css_forum_description = '';
            return $return;
        }
        ...
        function do_tweaks()
        {
            ...
            // fix forum font size
            if (!empty(self::get_option('font_size')))
            {
                add_action('wp_enqueue_scripts', array(&$this, 'action_custom_css_font_size'));
            }
        ...
        function action_show_description_reply()
        {
            ...
            foreach ($parent as $parent_id)
            {
                if (get_post_type($parent_id) === 'topic')
                {
                    ...
                    . '</div>'
                    . self::css_once_forum_description();
                }
                else
                {
                    $this->_action_show_description($parent_id);
        ...
        function _action_show_description($forum_id)
        {
            ...
            {
                echo '<div class="bbwptw-description">' . $description . '</div>' . self::css_once_forum_description();
            }
        }
         ...
         */
        function callback_for_setting_up_scripts($hook)
        {

            // plugin settings page, widgets page
            $add_css = ('settings_page_' . self::ID === $hook) || ('widgets.php' === $hook);

            // any bbpress page
            // $add_css = $add_css || self::is_body_class('bbpress');
            // settings_page_bbpress-wp-tweaks
            if ($add_css)
            {
                // get current version wp + plugin
                global $wp_version;
                $ver = esc_attr($wp_version . '-' . self::VERSION);

                wp_enqueue_style(self::ID, plugins_url('style.css', __FILE__), array(), $ver);
            }
        }
        ...
        function filter_bbp_get_theme_compat_templates($templates)
        {
            ...
            if (!empty($wrapper))
            {
                if ($wrapper === 'custom_wrapper')
                {
                    ...
                    if (!empty($wrapper_custom))
                    {
                        $return[$wrapper_custom] = $wrapper_custom;
                    }
                }
                else
                {
        ...
        /**
         * Add font size fix to bbpress front end if enabled
         */
        function action_custom_css_font_size()
        {
            $font_size = self::get_option('font_size');
            ...
            if (isset($arr_font_size[$font_size]) && self::is_body_class('bbpress'))
            {

                $css_handle = self::ID . '-font-asdasdasd';

                $set_font_size = $arr_font_size[$font_size];
                $set_font_size_s = round($set_font_size * 0.9, 1);
                $set_font_size_xs = round($set_font_size_s * 0.9, 1);
                $set_font_size_2xs = round($set_font_size_xs * 0.9, 1);

                // 1. Register a new (empty) stylesheet handle if it doesn't exist
                //    No dependencies, no version, all media
                wp_register_style($css_handle, false, array(), null, 'all');

                // 2. Enqueue the registered stylesheet (now it will be loaded)
                wp_enqueue_style($css_handle);

                // 3. Add code to stylesheet
                wp_add_inline_style($css_handle, '
                   ...
                   {
                     font-size:' . $set_font_size . 'rem; line-height:1.3;
                   }
                   body #bbpress-forums .bbp-forum-info .bbp-forum-content, 
                   body #bbpress-forums p.bbp-topic-meta,
                   body div.bbp-breadcrumb, 
                   body div.bbp-topic-tags                   
                   {
                    font-size:' . $set_font_size_s . 'rem;
                   }
                   body span.bbp-admin-links a,
                   body #bbpress-forums .bbp-forums-list .bbp-forum
                   {
                    font-size:' . $set_font_size_2xs . 'rem;
                   }
        ...
        /**
         * Check if wp uses block theme
         */
        static public function is_block_theme()
        {
            if (is_null(self::$is_block_theme))
            {
                self::$is_block_theme = function_exists('wp_is_block_theme') && wp_is_block_theme();
            }
            return self::$is_block_theme;
        }

        /**
         * Check if body has given class:
         * 		bbpress					-> all bbpress pages
         * 		forum-archive			-> forum home page, all forums
         * 		forum, single-forum		-> forum child or parent
         * 		topic, single-topic		-> forum topic
         * 		bbp-user-page			-> forum profile page and subs
         * 		general: logged-in
         *
         * @param string $str
         * @return boolean
         */
        static public function is_body_class($str)
        {
            return !empty($str) && in_array($str, get_body_class(), true);
        }
        ...
        function options_page()
        {
            ...
            ?>
            <div class="wrap">
                <h1><?php _e('bbPress WP Tweaks', 'bbpress-wp-tweaks'); ?> <em class="bbwptw-header-info"> — by <a href="https://veppa.com/" target="_blank">veppa.com</a></em></h1>
                <?php
                ...
                <div class="bbwptw-side">
                    <div class="bbwptw-box">
                        ...
                        <h3><?php _e('More', 'bbpress-wp-tweaks'); ?></h3>
                        <ul>
                            <li><a href="https://veppa.com/improve-pagespeed/" target="_blank"><?php _e('Get PageSpeed Score 100 ~ video', 'bbpress-wp-tweaks'); ?></a></li>
                            <li><a href="https://veppa.com/share-button/" target="_blank"><?php _e('Fast Native Share Button ~ without plugin', 'bbpress-wp-tweaks'); ?></a></li>
                            <li><a href="https://veppa.com/http-requests-manager/" target="_blank"><?php _e('WP_HTTP Manager ~ admin speed', 'bbpress-wp-tweaks'); ?></a></li>
                        </ul>
        ...
        public function page_init()
        {
            ...
            add_settings_field(
                    'font_size', // ID
                    __('Font size', 'bbpress-wp-tweaks'), // Title
                    array($this, 'callback_font_size'), // Callback
                    'bbpress-wp-tweaks-admin', // Page
                    'tweaks' // Section
            );
        ...
        function sanitize($input)
        {
            ...
            // make sure wrapper custom text is not 0. It saves as 0 when FSE theme used.
            if (empty($new_input['wrapper_custom']))
            {
                $new_input['wrapper_custom'] = '';
            }
        ...
        public function callback_wrapper()
        {
            ...
            if (self::is_block_theme())
            {
                echo '<p class="attention">' . __('Not available for FSE (Full Site Editing) themes.', 'bbpress-wp-tweaks') . '</p>';
                return false;
            }
            ...
            // ATTENTION :::: $_default_wrapper_file variable used for getting wrappers in correct order. do not delete it.
         ...
         * Get the settings option array and print one of its values
         */
        public function callback_font_size()
        {
            $font_size = self::get_option('font_size', '');
            ?>
            <p>
                <select name="bbpress-wp-tweaks[font_size]" id="font_size">
                    <option value=""><?php echo __('- default -'); ?></option>
                    <option <?php selected($font_size, 'XL'); ?> value="XL"><?php echo __('XL'); ?></option>
                    <option <?php selected($font_size, 'L'); ?> value="L"><?php echo __('L'); ?></option>
                    <option <?php selected($font_size, 'M'); ?> value="M"><?php echo __('M'); ?></option>
                    <option <?php selected($font_size, 'S'); ?> value="S"><?php echo __('S'); ?></option>
                    <option <?php selected($font_size, 'XS'); ?> value="XS"><?php echo __('XS'); ?></option>
                </select>
            </p>
            <?php
        }
    ...
    class BbpressWpTweaks_Users extends WP_Widget
    {
        ...
        function widget($args, $instance)
        {
            ...
            // show inlince css once
            echo BbpressWpTweaks::css_once_users_widget();
        }
...
//add_action('after_setup_theme', create_function('', 'global $BbpressWpTweaks; $BbpressWpTweaks = new BbpressWpTweaks();'));
    add_action('after_setup_theme', array('BbpressWpTweaks', 'instance'));
